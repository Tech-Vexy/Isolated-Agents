...
def _capture_stderr(logger: AuditLogger, event_type: str, **kwargs) -> dict:
...
class TestStructuredLogEntry:
    ...
    def test_non_violation_event_emits_required_fields(self, event_type):
        logger = AuditLogger(log_output_path=None)
        entry = _capture_stderr(logger, event_type)
    ...
    def test_violation_event_emits_required_fields(self, event_type):
        logger = AuditLogger(log_output_path=None)
        payload = {"violation_type": "test_violation", "attempted_action": "read /etc/passwd"}
        entry = _capture_stderr(logger, event_type, payload=payload)
        for field in _REQUIRED_TOP_LEVEL_FIELDS:
            assert field in entry, f"Missing required field '{field}' for event '{event_type}'"

    def test_event_type_is_preserved(self):
        logger = AuditLogger(log_output_path=None)
        entry = _capture_stderr(logger, "container_created")
        assert entry["event_type"] == "container_created"

    def test_session_id_is_preserved(self):
        logger = AuditLogger(log_output_path=None)
        entry = _capture_stderr(logger, "agent_launched", session_id="my-session-42")
        assert entry["session_id"] == "my-session-42"

    def test_agent_id_is_preserved(self):
        logger = AuditLogger(log_output_path=None)
        entry = _capture_stderr(logger, "agent_launched", agent_id="my-agent-99")
        assert entry["agent_id"] == "my-agent-99"

    def test_payload_is_preserved(self):
        logger = AuditLogger(log_output_path=None)
        payload = {"key": "value", "count": 3}
        entry = _capture_stderr(logger, "container_created", payload=payload)
        assert entry["payload"] == payload

    def test_timestamp_is_iso8601_utc_string(self):
        logger = AuditLogger(log_output_path=None)
        entry = _capture_stderr(logger, "container_created")
        ts = entry["timestamp"]
    ...
    def test_output_is_valid_json(self):
        ...
        buf = io.StringIO()
        with unittest.mock.patch.object(sys, "stderr", buf):
            logger.log_event("container_created", "s1", "a1", {})
        raw = buf.getvalue()
        # Should not raise
    ...
    def test_all_standard_event_types_produce_output(self, event_type):
        logger = AuditLogger(log_output_path=None)
        payload = {}
        ...
        buf = io.StringIO()
        with unittest.mock.patch.object(sys, "stderr", buf):
            logger.log_event(event_type, "s1", "a1", payload)
...
class TestViolationEventFields:
    ...
    @pytest.mark.parametrize("event_type", sorted(VIOLATION_EVENT_TYPES))
    def test_violation_payload_contains_violation_type(self, event_type):
        logger = AuditLogger(log_output_path=None)
    ...
    @pytest.mark.parametrize("event_type", sorted(VIOLATION_EVENT_TYPES))
    def test_violation_payload_contains_attempted_action(self, event_type):
        logger = AuditLogger(log_output_path=None)
    ...
    @pytest.mark.parametrize("event_type", sorted(VIOLATION_EVENT_TYPES))
    def test_violation_entry_contains_timestamp(self, event_type):
        logger = AuditLogger(log_output_path=None)
        payload = {"violation_type": "v", "attempted_action": "a"}
        entry = _capture_stderr(logger, event_type, payload=payload)
    ...
    @pytest.mark.parametrize("event_type", sorted(VIOLATION_EVENT_TYPES))
    def test_violation_entry_contains_agent_id(self, event_type):
        logger = AuditLogger(log_output_path=None)
        payload = {"violation_type": "v", "attempted_action": "a"}
        entry = _capture_stderr(logger, event_type, agent_id="agent-xyz", payload=payload)
        assert entry["agent_id"] == "agent-xyz"

    def test_filesystem_access_denied_event(self):
        logger = AuditLogger(log_output_path=None)
        payload = {"violation_type": "filesystem_access", "attempted_action": "read /etc/shadow"}
        entry = _capture_stderr(logger, "filesystem_access_denied", payload=payload)
        assert entry["event_type"] == "filesystem_access_denied"
        assert entry["payload"]["violation_type"] == "filesystem_access"

    def test_network_connection_denied_event(self):
        logger = AuditLogger(log_output_path=None)
        payload = {"violation_type": "network_access", "attempted_action": "connect 1.2.3.4:80"}
        entry = _capture_stderr(logger, "network_connection_denied", payload=payload)
    ...
    def test_resource_limit_exceeded_event(self):
        ...
        entry = _capture_stderr(logger, "resource_limit_exceeded", payload=payload)
        assert entry["event_type"] == "resource_limit_exceeded"

    def test_privilege_escalation_attempt_event(self):
        logger = AuditLogger(log_output_path=None)
        payload = {"violation_type": "privilege_escalation", "attempted_action": "sudo su"}
        entry = _capture_stderr(logger, "privilege_escalation_attempt", payload=payload)
        assert entry["event_type"] == "privilege_escalation_attempt"

    def test_output_size_exceeded_event(self):
        logger = AuditLogger(log_output_path=None)
        payload = {"violation_type": "output_size", "attempted_action": "write 500MB"}
        entry = _capture_stderr(logger, "output_size_exceeded", payload=payload)
...
class TestLogRoutingToFile:
    ...
    def test_nothing_written_to_stderr_when_file_set(self, tmp_path):
        log_file = str(tmp_path / "audit.log")
        logger = AuditLogger(log_output_path=log_file)
        buf = io.StringIO()
        with unittest.mock.patch.object(sys, "stderr", buf):
            logger.log_event("agent_launched", "s1", "a1", {})
        assert buf.getvalue() == "", "Expected no output on stderr when log_output_path is set"

    def test_multiple_events_appended_to_file(self, tmp_path):
        log_file = str(tmp_path / "audit.log")
        logger = AuditLogger(log_output_path=log_file)
        logger.log_event("container_created", "s1", "a1", {})
        logger.log_event("agent_launched", "s1", "a1", {})
        logger.log_event("container_destroyed", "s1", "a1", {})
        with open(log_file, encoding="utf-8") as f:
    ...
    def test_file_entry_is_valid_json(self, tmp_path):
        log_file = str(tmp_path / "audit.log")
        logger = AuditLogger(log_output_path=log_file)
        logger.log_event("container_created", "s1", "a1", {"extra": "data"})
        with open(log_file, encoding="utf-8") as f:
            raw = f.read().strip()
        entry = json.loads(raw)
        assert entry["payload"] == {"extra": "data"}

    def test_file_entry_contains_all_required_fields(self, tmp_path):
        log_file = str(tmp_path / "audit.log")
        logger = AuditLogger(log_output_path=log_file)
        logger.log_event("agent_launched", "sess-abc", "agent-xyz", {})
        with open(log_file, encoding="utf-8") as f:
            entry = json.loads(f.read().strip())
        for field in _REQUIRED_TOP_LEVEL_FIELDS:
            assert field in entry

    def test_violation_event_written_to_file(self, tmp_path):
        log_file = str(tmp_path / "audit.log")
        logger = AuditLogger(log_output_path=log_file)
        payload = {"violation_type": "network_access", "attempted_action": "connect 8.8.8.8"}
        logger.log_event("network_connection_denied", "s1", "a1", payload)
        with open(log_file, encoding="utf-8") as f:
            entry = json.loads(f.read().strip())
        assert entry["event_type"] == "network_connection_denied"
...
class TestLogRoutingToStderr:
    ...
    def test_entry_written_to_stderr(self):
        logger = AuditLogger(log_output_path=None)
        buf = io.StringIO()
        with unittest.mock.patch.object(sys, "stderr", buf):
            logger.log_event("container_created", "s1", "a1", {})
    ...
    def test_stderr_entry_is_valid_json(self):
        logger = AuditLogger(log_output_path=None)
        buf = io.StringIO()
        with unittest.mock.patch.object(sys, "stderr", buf):
            logger.log_event("agent_launched", "s1", "a1", {})
    ...
    def test_default_constructor_routes_to_stderr(self):
    ...
    def test_explicit_none_routes_to_stderr(self):
        logger = AuditLogger(log_output_path=None)
        buf = io.StringIO()
        with unittest.mock.patch.object(sys, "stderr", buf):
            logger.log_event("container_destroyed", "s1", "a1", {})
...
class TestEmitMethod:
    ...
    def test_emit_writes_to_stderr(self):
        logger = AuditLogger(log_output_path=None)
        event = AuditEvent(

...
async def run_runtime(args):
    ...
    setup_logging(level=logging.DEBUG if args.debug else logging.INFO, structured=args.json)

    runtime = AgentRuntime(working_dir=args.workspace, runtime_id=args.id)
...
async def run_one_liner(args):
    ...
    runtime = AgentRuntime(working_dir=args.workspace, enable_audit_logs=args.json)
...
def main():
    ...
    server_parser.add_argument(
        "--workspace", default="./runtime_workspace", help="Path to workspace"
    )
    ...
    run_parser.add_argument(
        "--workspace", default="./tmp_workspace", help="Host workspace directory"
    )
    ...
    if args.subcommand == "run":
        ...
        from isolated_agents_sdk.cli import cmd_config_create, cmd_config_list, cmd_init, cmd_ps

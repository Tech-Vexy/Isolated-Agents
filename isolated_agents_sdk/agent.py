...
try:
    from isolated_agents_sdk.config import Config, AgentConfig, load_config
...
class Agent:
    ...
    def with_workspace(self, path: Union[str, Path]) -> Agent:
    ...
    def _build_policy(self) -> Policy:
        ...
        network_policy = NetworkPolicy(
            disabled=not self._network_enabled,
    ...
    async def run_async(
        ...
        **kwargs
        ...
        if not workspace_path:
            raise ValueError(
                "Workspace path must be set either in constructor or run_async() call"
            )

...
class _RuntimeRegistry:
    ...
    def __init__(self) -> None:
        self._instance: AgentRuntime | None = None
    ...
    async def get_async(self, working_dir: str | Path | None = None) -> AgentRuntime:
    ...
    def get_sync(self, working_dir: str | Path | None = None) -> AgentRuntime:
...
async def get_runtime_async(working_dir: str | Path | None = None) -> AgentRuntime:
    """Get or create the global AgentRuntime instance (async, thread-safe)."""
    return await _runtime_registry.get_async(working_dir)


def get_runtime(working_dir: str | Path | None = None) -> AgentRuntime:
...
class AgentRuntime:
    ...
    def __init__(
        ...
        self._spawn_socket_path: str | None = None
        self._spawn_server = None
        self._hitl_handler: Callable[[str, str, int], Awaitable[str]] | None = None
        ...
        self.session_manager = SessionManager(
            audit_logger=self.audit_logger, state_dir=self.state_dir
        )
        self.provisioner = ContainerProvisioner(audit_logger=self.audit_logger)
    ...
    async def _create_session_socket(self, session_id: str) -> str | None:
    ...
    async def _close_session_socket(self, session_id: str) -> str | None:
    ...
    def _check_db_access(self, session_id: str | None, db_id: str) -> bool:
    ...
    async def _handle_db_query(self, request: dict[str, Any], session_id: str) -> dict[str, Any]:
        ...
        if not self._check_db_access(session_id, db_id):
            return {
                "status": "error",
                "error": f"Access denied to database '{db_id}' for session '{session_id}'",
            }
    ...
    async def _handle_db_nosql(
        self, request: dict[str, Any], op: str, session_id: str
    ) -> dict[str, Any]:
    ...
    @property
    def spawn_socket_path(self) -> str | None:
    ...
    async def cancel_session(self, session_id: str) -> bool:
        ...
        await self.session_manager.complete_session(
            session_id, exit_code=1, error="Cancelled by runtime."
        )
        return True
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "telemetry": {
                "total_executions": self.audit_logger._metrics.get("total_runs", 0)
                if hasattr(self.audit_logger, "_metrics")
                else 0,
                "violation_count": self.audit_logger._metrics.get("violations", 0)
                if hasattr(self.audit_logger, "_metrics")
                else 0,
            },
        }

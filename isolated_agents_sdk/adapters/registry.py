...
class AdapterRegistry:
    ...
    def get_container_adapter(self, name: Optional[str] = None) -> ContainerRuntimeAdapter:
        ...
        with self._lock:
            ...
            if name is None:
                raise AdapterConfigurationError(
                    "No default container adapter configured"
                )
            
            if name not in self._container_adapters:
                raise AdapterNotFoundError(
                    f"Container adapter not found: {name}"
                )
    ...
    def get_storage_adapter(self, name: Optional[str] = None) -> StorageAdapter:
        ...
        with self._lock:
            ...
            if name is None:
                raise AdapterConfigurationError(
                    "No default storage adapter configured"
                )
            
            if name not in self._storage_adapters:
                raise AdapterNotFoundError(
                    f"Storage adapter not found: {name}"
                )
    ...
    def get_audit_adapter(self, name: Optional[str] = None) -> AuditAdapter:
        ...
        with self._lock:
            ...
            if name is None:
                raise AdapterConfigurationError(
                    "No default audit adapter configured"
                )
            
            if name not in self._audit_adapters:
                raise AdapterNotFoundError(
                    f"Audit adapter not found: {name}"
                )
    ...
    def get_policy_adapter(self, name: Optional[str] = None) -> PolicyValidator:
        ...
        with self._lock:
            ...
            if name is None:
                raise AdapterConfigurationError(
                    "No default policy adapter configured"
                )
            
            if name not in self._policy_adapters:
                raise AdapterNotFoundError(
                    f"Policy adapter not found: {name}"
                )
